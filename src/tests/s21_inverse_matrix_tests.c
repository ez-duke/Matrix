...
START_TEST(s21_inverse_matrix_test5) {
  ...
  validate.matrix[0][0] = -6 / 23.0, validate.matrix[0][1] = -1 / 23.0,
  validate.matrix[0][2] = 16 / 23.0;
  validate.matrix[1][0] = 7 / 23.0, validate.matrix[1][1] = 5 / 23.0,
  validate.matrix[1][2] = -11 / 23.0;
  validate.matrix[2][0] = 11 / 23.0, validate.matrix[2][1] = -2 / 23.0,
  validate.matrix[2][2] = -14 / 23.0;
...
START_TEST(s21_inverse_matrix_test6) {
  ...
  s21_create_matrix(1, 1, &validate);

  A.matrix[0][0] = 4;

  ck_assert_int_eq(s21_inverse_matrix(&A, &result), 0);

  validate.matrix[0][0] = 1 / 4.0;
...
Suite *s21_inverse_matrix_test_suite() {
  ...
  TCase *tc_case = tcase_create("s21_inverse_matrix_case");
  ...
  tcase_add_test(tc_case, s21_inverse_matrix_test4);
  tcase_add_test(tc_case, s21_inverse_matrix_test5);
  tcase_add_test(tc_case, s21_inverse_matrix_test6);
